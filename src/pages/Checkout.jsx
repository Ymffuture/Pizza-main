...
export default function Checkout() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // ONLY send what backend OrderCreate schema expects:
      // { items, delivery_address, phone }
      const payload = {
      ...
      // Beanie may serialize id as string or as PydanticObjectId object
      ...
      if (payMethod === "paystack") {
        try {
          ...
          toast.show({ type: "info", title: "Order placed", message: "Redirecting to tracker…" });
      ...
      toast.show({
        type: "error",
        title: status === 401 ? "Session expired" : status === 422 ? "Validation error" : "Order failed",
        message: msg,
      });
    }
  };
      ...
      <header className="co-header">
        <div className="co-header-inner">
          ...
          <div>
            ...
            <p className="co-subtitle">{items.length} item{items.length !== 1 ? "s" : ""} &middot; {formatCurrency(total)}</p>
          </div>
        </div>
      </header>
          ...
          <form onSubmit={handleSubmit} className="co-form">
            ...
            <button type="submit" disabled={loading} className="co-submit-btn">
              {loading ? (
                ...
                <><Banknote className="w-5 h-5" /> Place Order &middot; Cash on Delivery <ChevronRight className="w-4 h-4 co-arrow" /></>
              ) : (
                <><CreditCard className="w-5 h-5" /> Pay {formatCurrency(total)} Online <ChevronRight className="w-4 h-4 co-arrow" /></>
              )}
            </button>
...
const styles = `
  ...
  .co-header-inner { max-width:680px; margin:0 auto; padding:14px 20px; display:flex; align-items:center; gap:14px; }
  .co-back-btn { width:38px; height:38px; flex-shrink:0; background:rgba(255,248,231,0.05); border:1px solid var(--border); border-radius:10px; display:flex; align-items:center; justify-content:center; color:var(--muted); cursor:pointer; }
  .co-back-btn:hover { color:var(--text); border-color:rgba(255,199,44,0.3); }
  .co-title { font-family:'Bebas Neue',sans-serif; font-size:22px; letter-spacing:3px; color:var(--text); line-height:1; }
  ...
  .co-cash-note { display:flex; align-items:flex-start; gap:10px; background:rgba(255,199,44,0.07); border:1px solid rgba(255,199,44,0.18); border-radius:12px; padding:12px 14px; font-size:13px; color:var(--text); line-height:1.5; }
  ...
  @media (max-width:600px) { .co-body { padding:16px 12px; } }
`;
