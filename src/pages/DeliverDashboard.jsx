...
/* ── Status config ── */
// ✅ FIX: "active" and "offline" were missing. If the backend ever set a driver's
// status to either of those values, STATUS_CFG[profile.status] returned undefined,
// the component fell back to STATUS_CFG.pending ("Under Review"), and the
// isApproved guard (`profile.status === "approved"`) evaluated to false — hiding
// the toggle, stats, and orders sections entirely.
const STATUS_CFG = {
  ...
  active:    { label: "Active Driver", color: "#4ade80", bg: "rgba(74,222,128,0.1)",   border: "rgba(74,222,128,0.25)",   Icon: CheckCircle2 },
  offline:   { label: "Offline",       color: "#94a3b8", bg: "rgba(148,163,184,0.1)",  border: "rgba(148,163,184,0.25)",  Icon: Circle },
...
export default function DeliverDashboard() {
  ...
  const fetchAll = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (p.status === "approved") {
        // Fetch active delivery and wallet in parallel — both non-fatal if they fail
        const [activeRes, balanceRes] = await Promise.allSettled([
          getActiveDelivery(),
          getWalletBalance(),
        ]);

        if (activeRes.status === "fulfilled") {
          setActiveDelivery(activeRes.value.data?.active ? activeRes.value.data : null);
        }
        ...
        if (p.is_available) {
          try {
            ...
            const isNetwork = !ordErr?.response || ordErr?.code === "ERR_NETWORK" || ordErr?.code === "ECONNABORTED";
            const status = ordErr?.response?.status;
            if (isNetwork) {
              showToast("Server is waking up — orders will load shortly. Retry in 30s.", "info");
            } else if (status !== 403) {
              // 403 just means "go online first" — not worth showing as error
              const msg = ordErr?.response?.data?.detail || "Could not load available orders";
              showToast(msg, "error");
            }
            setAvailableOrders([]);
          }
      ...
      if (err?.response?.status === 404) {
        ...
        setError("Server is waking up (Render cold start ~30–60s). Tap Retry when ready.");
      } else {
  ...
  // ✅ FIX: also treat "active" and "offline" as approved-equivalent so the
  // stats, toggle, and orders sections render correctly for those statuses.
  const isApproved = ["approved", "active", "offline"].includes(profile?.status);
      ...
      <div className="dd-body">
            ...
            <div className="dd-card dd-profile-card">
              <div className="dd-profile-row">
                <div className="dd-avatar">
                  ...
                  <span
                    className="dd-avatar-dot"
                    style={{ background: profile?.is_available ? "#4ade80" : "rgba(255,248,231,0.2)" }}
                  />
                </div>
              ...
              <>
                ...
                <div className={`dd-toggle-card${profile?.is_available ? " dd-toggle-online" : ""}`}>
                  <div className="dd-toggle-info">
                    <div
                      className="dd-toggle-icon"
                      style={{
                        background: profile?.is_available ? "rgba(74,222,128,0.15)" : "rgba(255,248,231,0.07)",
                      }}
                    >
                      <Power
                        className="w-6 h-6"
                        style={{ color: profile?.is_available ? "#4ade80" : "rgba(255,248,231,0.4)" }}
                      />
                    </div>
                    <div>
                      <p className="dd-toggle-title">
                        {profile?.is_available ? "You're Online" : "You're Offline"}
                      </p>
                  ...
                  <div className="dd-card dd-active-delivery">
                      ...
                      {DELIVERY_STATUS_STEPS.map((step, i) => {
                        ...
                        return (
                          <div key={step.key} className="dd-step">
                            <div
                              className={`dd-step-dot${done ? " dd-step-done" : ""}${active ? " dd-step-active" : ""}`}
                            >
                              <StepIcon className="w-3.5 h-3.5" />
                            </div>
                            <span className={`dd-step-lbl${done ? " dd-step-lbl-done" : ""}`}>
                              {step.label}
                            </span>
                          </div>
                        );
                    ...
                    <div className="dd-delivery-info">
                      <div className="dd-delivery-row">
                        <User className="w-4 h-4 dd-info-icon" />
                        <span>{activeDelivery.customer_name}</span>
                      </div>
                      <div className="dd-delivery-row">
                        <MapPin className="w-4 h-4 dd-info-icon" />
                        <span>{activeDelivery.delivery_address}</span>
                      </div>
                      {activeDelivery.customer_phone && (
                        <div className="dd-delivery-row">
                          <Phone className="w-4 h-4 dd-info-icon" />
                          <a href={`tel:${activeDelivery.customer_phone}`} className="dd-phone-link">
                            {activeDelivery.customer_phone}
                          </a>
                        </div>
                      )}
                ...
                <div className="dd-quick-grid">
                  <button className="dd-quick-card" onClick={() => setActiveTab("orders")}>
                    <div className="dd-quick-icon" style={{ background: "rgba(218,41,28,0.12)", color: "#DA291C" }}>
                      <ShoppingBag className="w-5 h-5" />
                    </div>
                    <span className="dd-quick-label">Available Orders</span>
                    {availableOrders.length > 0 && (
                      <span className="dd-quick-badge">{availableOrders.length}</span>
                    )}
                  </button>
                  <button className="dd-quick-card" onClick={() => setActiveTab("wallet")}>
                    <div className="dd-quick-icon" style={{ background: "rgba(74,222,128,0.12)", color: "#4ade80" }}>
                      <Wallet className="w-5 h-5" />
                    </div>
                    <span className="dd-quick-label">My Wallet</span>
                  </button>
                  <Link to="/wallet" className="dd-quick-card">
                    <div className="dd-quick-icon" style={{ background: "rgba(96,165,250,0.12)", color: "#60a5fa" }}>
                      <DollarSign className="w-5 h-5" />
                    </div>
                    <span className="dd-quick-label">Withdraw</span>
                  </Link>
                  <button className="dd-quick-card" onClick={() => fetchAll(true)}>
                    <div className="dd-quick-icon" style={{ background: "rgba(255,199,44,0.12)", color: "#FFC72C" }}>
                      <RefreshCw className={`w-5 h-5${refreshing ? " dd-spin" : ""}`} />
                    </div>
        ...
        {activeTab === "orders" && (
          <>
            {!isApproved ? (
              ...
              <div className="dd-card">
                <div className="dd-section-lbl">
                  <Truck className="w-4 h-4" style={{ color: "#FFC72C" }} />
                  Active Delivery
                </div>
        ...
        {activeTab === "wallet" && (
          <>
            <div className="dd-card">
              <div className="dd-section-lbl">
                <Wallet className="w-4 h-4" style={{ color: "#FFC72C" }} />
                Wallet Overview
              </div>
              ...
              <div className="dd-wallet-row">
                <span>Total Earned</span>
                <span style={{ color: "#4ade80", fontWeight: 700 }}>R{walletBalance?.total_earned?.toFixed(2) || "0.00"}</span>
              </div>
              <div className="dd-wallet-row">
                <span>Total Withdrawn</span>
                <span style={{ color: "#f87171", fontWeight: 700 }}>R{walletBalance?.total_withdrawn?.toFixed(2) || "0.00"}</span>
              </div>
              {walletBalance?.pending_amount > 0 && (
                <div className="dd-wallet-row">
                  <span>Pending</span>
                  <span style={{ color: "#FFC72C", fontWeight: 700 }}>R{walletBalance.pending_amount?.toFixed(2)}</span>
                </div>
              )}
            ...
            <div className="dd-card">
              <div className="dd-section-lbl" style={{ marginBottom: 4 }}>
...
const styles = `
  ...
  .dd-header {
    ...
    background: rgba(14,7,0,0.95); backdrop-filter: blur(20px);
  ...
  .dd-tabs-wrap {
    ...
    background: rgba(14,7,0,0.9); backdrop-filter: blur(10px);
  ...
  .dd-tab-badge {
    ...
    font-size: 10px; font-weight: 900; border-radius: 9px;
    display: flex; align-items: center; justify-content: center;
  }
  .dd-refresh-btn {
  ...
  .dd-avatar {
    ...
    background: rgba(255,199,44,0.12); border: 2px solid rgba(255,199,44,0.25);
    display: flex; align-items: center; justify-content: center;
    overflow: hidden; position: relative;
  }
  .dd-avatar-img { width: 100%; height: 100%; object-fit: cover; }
  .dd-avatar-initials {
    font-family: 'Bebas Neue', sans-serif; font-size: 22px; letter-spacing: 1px; color: var(--gold);
  }
  .dd-avatar-dot {
  ...
  .dd-meta-chip {
    ...
    background: rgba(255,248,231,0.06); border: 1px solid var(--border); color: var(--muted);
  }

  /* Status banners */
  .dd-status-banner {
    display: flex; align-items: flex-start; gap: 10px;
    padding: 12px 14px; border-radius: 12px; border: 1px solid; font-size: 13px;
  }
  ...
  .dd-toggle-btn {
    padding: 10px 20px; border-radius: 50px; flex-shrink: 0;
    background: rgba(255,248,231,0.07); border: 1.5px solid var(--border);
  ...
  .dd-delivery-steps::before {
    content: ''; position: absolute; top: 17px; left: 16px; right: 16px;
    height: 2px; background: rgba(255,248,231,0.08);
  }
  .dd-step { display: flex; flex-direction: column; align-items: center; gap: 6px; z-index: 1; }
  .dd-step-dot {
    width: 34px; height: 34px; border-radius: 50%;
    background: rgba(255,248,231,0.06); border: 2px solid rgba(255,248,231,0.1);
  ...
  .dd-delivery-action-btn {
    display: flex; align-items: center; justify-content: center; gap: 8px;
    background: var(--gold); color: #0e0700; border: none; cursor: pointer;
    font-family: 'Plus Jakarta Sans', sans-serif; font-weight: 900; font-size: 14px;
    padding: 13px; border-radius: 12px; transition: all 0.2s;
  }
  ...
  .dd-quick-card {
    ...
    cursor: pointer; text-decoration: none; transition: all 0.2s; color: var(--muted);
    position: relative;
  }
  ...
  .dd-timeline-num {
    ...
    color: rgba(255,199,44,0.2); line-height: 1; flex-shrink: 0;
    min-width: 36px;
  }
  ...
  .dd-accept-btn {
    ...
    box-shadow: 0 4px 14px rgba(218,41,28,0.35); flex-shrink: 0;
  }
  ...
  .dd-empty-state {
    ...
    gap: 10px; text-align: center; padding: 60px 20px;
    background: var(--card); border: 1px solid var(--border); border-radius: 18px;
  }
  .dd-empty-state p { font-size: 15px; font-weight: 800; color: var(--text); }
  .dd-empty-state span { font-size: 13px; color: var(--muted); max-width: 240px; line-height: 1.5; }
  .dd-go-online-btn {
    display: flex; align-items: center; gap: 8px;
    ...
    font-weight: 800; font-size: 13px; padding: 10px 22px; border-radius: 50px;
  ...
  .dd-wallet-action-btn {
    display: flex; align-items: center; gap: 10px;
    background: rgba(255,199,44,0.08); border: 1px solid rgba(255,199,44,0.2);
    color: var(--gold); text-decoration: none;
    font-family: 'Plus Jakarta Sans', sans-serif; font-weight: 800; font-size: 13px;
    padding: 12px 16px; border-radius: 12px; transition: all 0.2s; margin-top: 4px;
  }
