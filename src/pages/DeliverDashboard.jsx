...
// FIX: treat approved/active/offline as all valid post-approval states
const APPROVED_STATUSES = new Set(["approved", "active", "offline"]);
...
export default function DeliverDashboard() {
  ...
  const fetchAll = useCallback(async (silent = false) => {
    ...
    try {
      ...
      // FIX: was checking p.status === "approved" only — blocked active/offline drivers
      if (APPROVED_STATUSES.has(p.status)) {
  ...
  const handleAccept = async (orderId) => {
    ...
    try {
      ...
      if (status === 409) showToast("Another driver already accepted this order.", "error");
      else if (status === 400) showToast(detail || "Cannot accept this order right now.", "error");
      else if (status === 403) showToast("Go online first to accept orders.", "error");
      else showToast(detail || err?.message || "Failed to accept order — try again.", "error");
      await fetchAll(true);
    } finally {
      setAcceptingId(null);
    }
  ...
  const handleUpdateStatus = async (assignmentId, status) => {
    ...
    try {
      ...
      const labels = {
        ...
        delivered:  "Delivered! Earnings credited.",
      };
  ...
  const isApproved = APPROVED_STATUSES.has(profile?.status);
    ...
    <div className="dd-root">
      ...
      {toast && (
        <div key={toast.id} className={`dd-toast dd-toast-${toast.type}`}>
          {toast.type === "error"   ? <AlertCircle className="w-4 h-4" style={{ flexShrink: 0 }} />
           : toast.type === "info" ? <Zap          className="w-4 h-4" style={{ flexShrink: 0 }} />
              ...
              <div className="dd-profile-row">
                ...
                <div className="dd-profile-info">
                  ...
                  <div className="dd-profile-meta">
                    <span className="dd-meta-chip">🚗 {profile?.vehicle_type?.charAt(0).toUpperCase() + profile?.vehicle_type?.slice(1)}</span>
                    <span className="dd-meta-chip">⭐ {profile?.rating?.toFixed(1)}</span>
                  </div>
                </div>
              </div>
            ...
            {isApproved && (
              <>
                ...
                <div className="dd-stats-grid">
                  {[
                    { icon: <DollarSign className="w-5 h-5" />, val: `R${walletBalance?.balance?.toFixed(2) || "0.00"}`, lbl: "Balance", bg: "rgba(255,199,44,0.12)", c: "#FFC72C" },
                    { icon: <TrendingUp className="w-5 h-5" />, val: `R${walletBalance?.total_earned?.toFixed(2) || "0.00"}`, lbl: "Total Earned", bg: "rgba(74,222,128,0.12)", c: "#4ade80" },
                    { icon: <Bike className="w-5 h-5" />,       val: profile?.total_deliveries || 0, lbl: "Deliveries", bg: "rgba(96,165,250,0.12)", c: "#60a5fa" },
                    { icon: <Star className="w-5 h-5" />,       val: profile?.rating?.toFixed(1) || "5.0", lbl: "Rating", bg: "rgba(251,191,36,0.12)", c: "#fbbf24" },
                  ].map(({ icon, val, lbl, bg, c }) => (
                    <div key={lbl} className="dd-stat-card">
                      <div className="dd-stat-icon" style={{ background: bg, color: c }}>{icon}</div>
                      <span className="dd-stat-val">{val}</span>
                      <span className="dd-stat-lbl">{lbl}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className={`dd-toggle-card${profile?.is_available ? " dd-toggle-online" : ""}`}>
                  <div className="dd-toggle-info">
                    ...
                    <div>
                      ...
                      <p className="dd-toggle-sub">
                        {profile?.is_available ? "Accepting orders — stay ready!" : "Go online to receive delivery requests"}
                      </p>
                    </div>
                  </div>
                  <button
                  ...
                  >
                    {toggling ? <Loader2 className="w-5 h-5 dd-spin" /> : profile?.is_available ? "Go Offline" : "Go Online"}
                  </button>
                </div>
                ...
                {activeDelivery && (
                  <div className="dd-card dd-active-delivery">
                    ...
                    {/* ✅ FIXED: Video Call Button - properly placed outside steps loop */}
                ...
                <div className="dd-quick-grid">
                  <button className="dd-quick-card" onClick={() => setActiveTab("orders")}>
                    ...
                    <span className="dd-quick-label">Orders</span>
                  ...
                  <button className="dd-quick-card" onClick={() => setActiveTab("wallet")}>
                    ...
                    <span className="dd-quick-label">Wallet</span>
                  </button>
            ...
            {!isApproved ? (
              ...
              <div className="dd-empty-state">
                ...
                <button className="dd-go-online-btn" onClick={handleToggle} disabled={toggling}>
                  {toggling ? <Loader2 className="w-4 h-4 dd-spin" /> : <Power className="w-4 h-4" />} Go Online
                </button>
              </div>
            ) : activeDelivery ? (
              <div className="dd-card">
              ...
              <div className="dd-empty-state">
                ...
                <span>Orders appear once admin marks them <strong style={{ color: "#a78bfa" }}>Ready</strong>. Auto-refreshes every 15s.</span>
                <button className="dd-go-online-btn" onClick={() => fetchAll(true)} disabled={refreshing}>
                  <RefreshCw className={`w-4 h-4${refreshing ? " dd-spin" : ""}`} /> Refresh
                </button>
              </div>
            ) : (
        ...
        {activeTab === "wallet" && (
          <>
            <div className="dd-card">
              ...
              {[
                { lbl: "Total Earned",    val: `R${walletBalance?.total_earned?.toFixed(2)    || "0.00"}`, c: "#4ade80" },
                { lbl: "Total Withdrawn", val: `R${walletBalance?.total_withdrawn?.toFixed(2) || "0.00"}`, c: "#f87171" },
              ].map(({ lbl, val, c }) => (
                <div key={lbl} className="dd-wallet-row">
                  <span>{lbl}</span><span style={{ fontWeight: 700, color: c }}>{val}</span>
                </div>
              ))}
              {walletBalance?.pending_amount > 0 && (
                <div className="dd-wallet-row">
                  <span>Pending</span>
                  <span style={{ fontWeight: 700, color: "#FFC72C" }}>R{walletBalance.pending_amount?.toFixed(2)}</span>
                </div>
              )}
              <Link to="/wallet" className="dd-wallet-action-btn">
                <DollarSign className="w-4 h-4" /> Full Wallet & Withdraw
                <ChevronRight className="w-4 h-4" style={{ marginLeft: "auto" }} />
              </Link>
            </div>

            <div className="dd-card">
              <div className="dd-section-lbl"><TrendingUp className="w-4 h-4" style={{ color: "#FFC72C" }} /> Earnings Info</div>
              {[
                { lbl: "Per delivery",        val: "R15.00"   },
                { lbl: "Minimum withdrawal",  val: "R50.00"   },
                { lbl: "Payout time",         val: "24–48 hrs" },
              ].map(({ lbl, val }) => (
                <div key={lbl} className="dd-wallet-row">
                  <span>{lbl}</span><span style={{ fontWeight: 700, color: "var(--text)" }}>{val}</span>
                </div>
              ))}
            </div>
          </>
...
const styles = `
  ...
  .dd-tabs-wrap{display:flex;align-items:center;gap:0;background:rgba(14,7,0,0.9);backdrop-filter:blur(10px);border-bottom:1px solid var(--border);padding:0 16px;overflow-x:auto;scrollbar-width:none;}
  ...
  .dd-stat-lbl{font-size:10px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:0.06em;text-align:center;}
  ...
  @media(max-width:480px){.dd-stats-grid{grid-template-columns:repeat(2,1fr);}.dd-quick-grid{grid-template-columns:repeat(2,1fr);}.dd-body{padding:16px 12px;}.dd-toast{bottom:16px;max-width:calc(100vw - 32px);white-space:normal;}}
`;
