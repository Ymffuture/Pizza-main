...
import {
  ...
  Navigation, CheckCheck, Circle, Zap, LogOut, WifiOff
} from "lucide-react";
...
export default function DeliverDashboard() {
  ...
  // Track when we last fetched orders to prevent spam
  ...
  // ── Fetch available orders (separate function) ──
  const fetchOrdersOnly = useCallback(async (silent = false) => {
    ...
    try {
      ...
      console.log("❌ Orders fetch error:", err?.response?.status, err?.message);
      ...
      if (isNetwork) {
        ...
        const msg = err?.response?.data?.detail || "Could not load orders";
        showToast(msg, "error");
      }
      setAvailableOrders([]);
      return [];
  ...
  const fetchAll = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (p.status === "approved") {
        // Fetch active delivery and wallet in parallel
        ...
        // CRITICAL: Always fetch orders if online, no active delivery, and not silent refresh
        if (p.is_available && !activeRes.value?.data?.active) {
          await fetchOrdersOnly(true);
        } else {
          setAvailableOrders([]);
        }
      }
  ...
  // ── CRITICAL FIX: Fetch orders when switching to Orders tab ──
  useEffect(() => {
    if (activeTab === "orders" && profile?.is_available && !activeDelivery) {
      // Always refresh when opening orders tab (but throttle to 3s min)
      const timeSinceLastFetch = Date.now() - lastFetchRef.current;
      if (timeSinceLastFetch > 3000) {
        console.log("📱 Orders tab opened, fetching orders...");
        fetchOrdersOnly(true);
      }
    }
  }, [activeTab, profile?.is_available, activeDelivery, fetchOrdersOnly]);

  // ── Auto-refresh every 10s when online and on orders tab ──
  useEffect(() => {
    if (!profile?.is_available || activeDelivery || activeTab !== "orders") return;
    
    console.log("🔄 Starting auto-refresh for orders...");
  ...
  }, [profile?.is_available, activeDelivery, activeTab, fetchOrdersOnly]);
  ...
  const handleToggle = async () => {
    ...
    try {
      ...
      const newAvailable = res.data.is_available;
      setProfile(p => ({ ...p, is_available: newAvailable }));
      
      if (newAvailable) {
        // IMMEDIATELY fetch orders when going online
        const orders = await fetchOrdersOnly(false);
        if (orders.length === 0) {
          showToast("You're online! Waiting for orders…", "info");
        } else {
          showToast(`You're online! ${orders.length} order(s) available`, "success");
        }
      } else {
        setAvailableOrders([]);
    ...
    return (
      <div className="dd-root">
        ...
        <div className="dd-nodriver">
          <div className="dd-nodriver-icon" style={{ background: "rgba(248,113,113,0.1)", borderColor: "rgba(248,113,113,0.2)" }}>
            <WifiOff className="w-10 h-10" style={{ color: "#f87171" }} />
          </div>
          <h2 className="dd-nodriver-title">Connection Lost</h2>
          <p className="dd-nodriver-sub">Cannot connect to server. Check your internet or wait for server to wake up.</p>
          <button onClick={() => fetchAll()} className="dd-nodriver-btn">Retry Connection</button>
        </div>
      </div>
    );
  }
    ...
    <div className="dd-root">
      ...
      <div className="dd-tabs-wrap">
        ...
        <button
          ...
          onClick={() => activeTab === "orders" ? fetchOrdersOnly() : fetchAll(true)}
          disabled={refreshing}
          title="Refresh"
        >
                    ...
                    <div>
                      ...
                      <p className="dd-toggle-sub">
                        {profile?.is_available
                          ? `Accepting orders — ${availableOrders.length} available`
          ...
          <>
            {!isApproved ? (
              ...
              <div className="dd-empty-state">
                ...
                <span>Orders appear here once the admin marks them <strong style={{color:"#a78bfa"}}>Ready</strong>. Stay online!</span>
                <button className="dd-go-online-btn" onClick={() => fetchOrdersOnly()} disabled={refreshing}>
                  <RefreshCw className={`w-4 h-4${refreshing ? " dd-spin" : ""}`} />
                  Check for Orders
                </button>
                <div style={{marginTop:8,padding:"10px 16px",borderRadius:12,background:"rgba(255,199,44,0.07)",border:"1px solid rgba(255,199,44,0.2)",fontSize:11,color:"rgba(255,248,231,0.5)",textAlign:"center",maxWidth:280}}>
                  Auto-checking every 10 seconds while you're on this tab
                </div>
              </div>
            ) : (
              <div className="dd-orders-list">
                <div className="dd-section-lbl" style={{ padding: "0 0 4px" }}>
                  ...
                  <span style={{marginLeft:"auto",fontSize:10,color:"var(--muted)",fontWeight:600}}>Live updates</span>
                </div>
