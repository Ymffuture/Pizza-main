...
export default function Pricing() {
  ...
  const {
    isProBite, billingCycle: currentCycle, expiresAt,
    cancelAtPeriodEnd, credits, refresh,
  } = useBilling();
  ...
  /* ── Load plan pricing — public, no auth required ── */
  useEffect(() => {
    getPlans()
  ...
  /* ── Post-Paystack redirect: ?reference=probite-monthly-xxxx ── */
  ...
  const handleSubscribe = useCallback(async () => {
    ...
    try {
      ...
      toast.show({
        ...
        message: typeof detail === "string" ? detail : "Please try again in a moment.",
      });
    ...
    <div className="pr-root">
      ...
      <div className="pr-wrap">
        ...
        <div className="pr-brand">
          <div className="pr-brand-badge"><Flame className="w-5 h-5" style={{ color: "#0e0700" }} /></div>
          <span className="pr-brand-name">KOTABITES</span>
        </div>

        <h1 className="pr-title">Simple pricing, unlimited bites</h1>
        <p className="pr-sub">
          Free gets you a generous taste of KotaBot. ProBite removes the limits entirely.
        </p>

        {/* Verify state — shown once, right under the header */}
        {verifying && (
        ...
        <div className="pr-toggle">
          <button
            className={`pr-toggle-btn ${cycle === "monthly" ? "pr-toggle-active" : ""}`}
            onClick={() => setCycle("monthly")}
          >
            Monthly
          </button>
          <button
            className={`pr-toggle-btn ${cycle === "yearly" ? "pr-toggle-active" : ""}`}
              ...
              <div className="pr-current-plan">
                <div className="pr-current-row">
                  ...
                  <span>
                    ...
                    {expiresAt ? new Date(expiresAt).toLocaleDateString("en-ZA", { day: "numeric", month: "short", year: "numeric" }) : "—"}
                  </span>
                </div>
                {!cancelAtPeriodEnd && (
        ...
        {/* Current free-plan credit status, only when relevant */}
        {!isProBite && isAuth && (
          <p className="pr-credit-status">
            You currently have <strong>{credits.unlimited ? "unlimited" : `${credits.credits ?? "—"}/${credits.creditsCap ?? 20}`}</strong> KotaBot credits.
          </p>
        )}
...
const styles = `
  ...
  .pr-wrap { position: relative; z-index: 1; max-width: 880px; margin: 0 auto; display: flex; flex-direction: column; align-items: center; text-align: center; }
  ...
  .pr-cards { display: grid; grid-template-columns: 1fr 1fr; gap: 18px; width: 100%; margin-bottom: 28px; }
